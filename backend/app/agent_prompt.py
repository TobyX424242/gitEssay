...
def build_system_prompt(
    ...
    lines = [
        ...
        "- Equations: a [[EQ:nonce]] token in the text is a LaTeX equation block — the document's equation listing shows the raw LaTeX behind each token, so read it there (never guess its content). To CHANGE an equation's LaTeX, use an equation edit {\"equation\": nonce, \"latex\": new_latex} in propose_patch; text edits must keep the token verbatim and can only move/keep/remove the whole equation, never its content. New LaTeX must parse under KaTeX.",
        "- To ADD brand-new content (a new section, paragraph, or conclusion), use an append edit {\"append\": text} in propose_patch — it lands at the end of the document as new paragraph(s). You may include NEW display equations in the appended text as LaTeX fenced by lines containing only $$; the LaTeX must parse under KaTeX. Use search/replace edits for changing existing text; do not append what you could edit in place.",
